...
class DatosPM25View(FlaskView):
    ...
    def primer_año_all(self):
        estaciones = mongo.db.datospmdos_uno.find()
    ...
    def primer_año_byes(self):
        ...
        estaciones = mongo.db.datospmdos_uno.find({}, {'_id' : 0, estacion : 1, 
                                                        'FECHA' : 1, 'HORA' : 1})
        response = json_util.dumps(estaciones)
    ...
    def segundo_año_all(self):
        estaciones = mongo.db.datospmdos_dos.find()
        response = json_util.dumps(estaciones)
        return Response(response, mimetype="application/json"), HTTPStatus.OK
    ...
    def segundo_año_byes(self):
        ...
        estaciones = mongo.db.datospmdos_dos.find({}, {'_id' : 0, estacion : 1, 
                                                        'FECHA' : 1, 'HORA' : 1})
        response = json_util.dumps(estaciones)
